...
class FlickrPopularRequest {
    ...
    class func fetchPhotosForRequest( searchText: String, onCompletion: @escaping FlickrResponse) -> Void {
        ...
        let searchTask = URLSession.shared.dataTask(with: url as URL, completionHandler: {data, response, error -> Void in
                        if error != nil {
                print("Error fetching photos: \(error ?? 0 as! Error)")
